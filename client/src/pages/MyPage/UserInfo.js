...
function UserInfo() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.REACT_APP_SERVER_URL}/mypage/users`, {
  ...
  useEffect(() => {
    // disabled 조건문인데 확인 잘하자 이걸로 함수 실행하자 근데 이게 최선인가
    if (curUserPw) {
  ...
  useEffect(() => {}, [fixUserName, curUserPw, updatePw, checkUpdatePw]);

  function updateInfoRequest(e) {
    e.preventDefault();
    const userData2 = {
      userName: fixUserName,
      editPassword: updatePw,
      password: curUserPw,
    };

    axios
      .patch(`${process.env.REACT_APP_SERVER_URL}/mypage/users`, userData2, {
        headers: { authorization: `Bearer ${userData}`, withCredentials: true },
      })
      .then((res) => {
        console.log(res);
        setSucUpdate(true);
  ...
  return (
    <UserInfoBackground>
      <UserInfoContainer>
        ...
        <UserInfoUpdate>
          <UserInfoForm onSubmit={(e) => updateInfoRequest(e)}>
            ...
            <UserInfoBox>
              <UserInfoLabel htmlFor="curPassword">
                현재 비밀번호 입력
              </UserInfoLabel>
              <UserInfoPwd
                autocomplete="current-password"
                onChange={(e) => userPwInfo(e)}
                value={curUserPw || ""}
              />
            </UserInfoBox>
            <UserInfoBox>
              <UserInfoLabel htmlFor="changePassword">
                새 비밀번호 입력
              </UserInfoLabel>
              {/* 필수 아님 */}
              <UserInfoEditPwd
                autocomplete="new-password"
                onChange={(e) => chagePwInfo(e)}
                value={updatePw || ""}
              />
            </UserInfoBox>
            {!validPw ? (
              <CheckSignMsg
                message={"숫자, 영문, 특수문자 각 1자리 이상의 8~16 자리"}
              />
            ) : null}
            <UserInfoBox>
              <UserInfoLabel htmlFor="checkPassword">새 비밀번호</UserInfoLabel>
              <UserInfoChkPassword
                onChange={(e) => checkChagePwInfo(e)}
                value={checkUpdatePw || ""}
              />
            </UserInfoBox>
            {!samePw ? (
              <CheckSignMsg message={"비밀번호가 일치하지 않습니다."} />
            ) : null}
      ...
      {showWithDrawal ? (
        <WithDrawalModal setShowWithDrawal={setShowWithDrawal} />
      ) : null}
    </UserInfoBackground>
  );
}
