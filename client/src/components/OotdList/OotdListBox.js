...
function OotdListBox() {
  ...
  const imageModalHandler = (handle) => {
    dispatch(isShowOotdImageModalHandler(handle));
  };
  const isShowImageModal = useSelector(
    (state) => state.isShowModalReducer.isShowOotdImageModal
  );
  ...
  const ootdListGrid = useCallback((column, width) => {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      if (i === images.length - 1 && ootdListContainer.current) {
        ootdListContainer.current.style.height = `${Math.max.apply(
          0,
          imgStack
        )}px`;
      }
  ...
  const getOotdList = () => {
    ...
    axios
      ...
      .then((result) => {
        ...
        setOotdListArray((ootdListArray) => [
          ...ootdListArray,
          ...result.data[0],
        ]);
      })
    ...
    <OotdListBoxContainer ref={ootdListContainer} className="ootd-list">
      {ootdListArray.map((val, idx) => {
        return (
          <OotdListBoxBack
            ref={ootdImageBox}
            className="ootd-image-box"
            key={idx}
            onClick={(e) => clickedImage(e)}
          >
            <OotdListBoxOver>
              <OotdListBoxImage
                src={`${val.diariesImage}`}
                className="ootdImage"
              />
              <OotdListBoxItem className="ootd-list-box-item"></OotdListBoxItem>
              <OotdLikeCont
                likeCounts={val.likeCounts}
                likeWhether={val.likeWhether ? val.likeWhether : false}
                diariesId={val.diariesId}
                likeClass={"like-container"}
              />
              <DiaryUserName className="diary-user-name">
                {val.userName}
              </DiaryUserName>{" "}
              {/* 작성자 이름 */}
            </OotdListBoxOver>
            <OotdHashtags className="ootd-hashtags">
              {val.hashtag
                ? val.hashtag.split(",").map((val, idx) => {
                    return <OotdHashtag key={idx}>#{val}</OotdHashtag>;
                  })
                : null}
            </OotdHashtags>
          </OotdListBoxBack>
      ...
      {isShowImageModal ? (
        <OotdImageModal modalImage={modalImage} setModalImage={setModalImage} />
      ) : null}
    </OotdListBoxContainer>
  );
}
